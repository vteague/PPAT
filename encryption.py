...
def KeyGen(pp):
    ...
    def pi_t(gt):
        if gt[1] == 1:
            ...
            output = Fp12.mul(gt[0],Fp12.mul(Fp12.powop(gt[1],s),Fp12.powop(Fp12.powop(gt[2],s),s))) # pi_t((gt1,gt2,gt3)) = gt1 * gt2^s * gt3^{s^2}
        return output 
...
def Multiply_src(pk,C0,C1): 
    ...
    #e(C0,C1):
    eC = (e_hat(C0[0],C1[0],Pair),Fp12.mul(e_hat(C0[0],C1[1],Pair),e_hat(C0[1],C1[0],Pair)),e_hat(C0[1],C1[1],Pair)) 
    #e(g,h1):
    g1 = (e_hat(g,EFp2.dbleAndAdd(H1[0],randint(1,int(p))),Pair),e_hat(g,EFp2.dbleAndAdd(H1[1],randint(1,int(p))),Pair),Fp12.one())
    #e(g1,h):
    h1 = (e_hat(EFp.dbleAndAdd(G1[0],randint(1,int(p))),h,Pair),e_hat(EFp.dbleAndAdd(G1[1],randint(1,int(p))),h,Pair),Fp12.one())
